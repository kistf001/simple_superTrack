...
from utils.network import PolicyWorld

import utils.env as env
import utils.buffer as buffer
import learning.learningMLT as learningMLT
import learning.learningSGL as learningSGL
...
if __name__ == "__main__":
    ...
    # model setting
    ####################################################################
    model_master = PolicyWorld(local_dim=244, state_dim=(16 * 3) + (16 * 3), control_dim=env.get_info()["control_size"])
    model_master.to(device)
    # model_master.load("./train_weight")

    model_slave = PolicyWorld(local_dim=244, state_dim=(16 * 3) + (16 * 3), control_dim=env.get_info()["control_size"])
    ...
    if buffer.buffer_size_processor == 1:
        learningSGL.start(model_master, model_slave, L1_loss, L2_loss, optim_world, optim_policy, buffer, env)

    # MULTI PROCESS
    else:
        learningMLT.start(model_master, model_slave, L1_loss, L2_loss, optim_world, optim_policy, buffer, env)
