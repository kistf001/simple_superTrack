...
def step_backend(pos, quat, pos_prv, quat_prv):
    ...
    buffer = (
        ...
        quat.clone(),
        myQuaternion.quat_differentiate_angular_velocity(
            quat, quat_prv, torch.tensor([100])).clone()
        )
    pos_prv = pos.clone()
    quat_prv = quat.clone()
    return buffer, pos_prv, quat_prv
...
def reset():
    ...
    mujoco.mj_resetData(m, d), mujoco.mj_step(m, d)
...
def step(ctrl=None):
    ...
    buffer, pos_prv, quat_prv = step_backend( 
        torch.from_numpy(d.xpos [1:]),
        torch.from_numpy(d.xquat[1:]),
        pos_prv, 
        quat_prv
        )
    return buffer
